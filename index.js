...
next();
 ...
 // Job Posting API (Updated with error handling)
 app.post('/jobs', verifyToken, async (req, res) => {
  const job = req.body;
  if (!job.tutorId || !job.subject || !job.description || !job.email) {
    return res.status(400).send({ message: 'Missing required fields' });
  }
  try {
    const tutor = await tutorCollection.findOne({ _id: new ObjectId(job.tutorId) });
    if (!tutor) {
...
// Send Message to Tutor (Updated with error handling)
app.post('/send-message', verifyToken, async (req, res) => {
  const { tutorEmail, message } = req.body;
  if (!tutorEmail || !message) {
    return res.status(400).send({ message: 'Tutor email and message are required' });
  }
  try {
...
//     const userCollection = client.db("LesonPaw-user").collection("users");
//     const jobCollection = client.db("LesonPaw-jobs").collection("jobs");
//     const tutorCollection = client.db("LesonPaw-tutors").collection("tutors");
...
//       const filter = { email };
//       const result = await userCollection.updateOne(filter, { $set: update });
//       res.send(result);
//     });
